	...
	if ( $_SERVER[ "REQUEST_METHOD" ] == "GET" )
	{
		...
		if ( $method == "placeholder" )
		{
			...
			if($width < 210)
			{
				...
				$image->spliceImage(20, 0, $width, 0);
			}
		...
		else
		{
			...
			//UploadRackspaceImage($file_name);
		}
	}
}
...
function ProcessDlRequest()
{
	...
	switch ( $_POST[ "action" ] )
	{
		...
		case "email":
		{
			...
			//$from = "UC Web Team <ucwebteam2@spu.edu>";
			$from = "ucwebteam2@spu.edu";
			$to = $_POST[ "rcpt" ];
			$subject = $_POST[ "rcpt" ];
			$body = $html;

			$host = "alertmail.spu.edu";
		...
		case "save":
		{
			$vegan_html = $_POST[ "html" ];
			$filename = "saved-templates/" . $_POST[ "filename" ] . ".content";
			$backup_filename = "saved-templates/" . $_POST[ "filename" ] . ".backup";
			$meta_filename = "saved-templates/" . $_POST[ "filename" ] . ".meta";
			try{
					$myfile = fopen($filename, "w");
					fwrite($myfile, $vegan_html);
					fclose($myfile);

					$theirfile = fopen($backup_filename, "w");
					fwrite($theirfile, $vegan_html);
					fclose($theirfile);

					$yourfile = fopen($meta_filename, "w");
					fwrite($yourfile, $_POST[ "metadata" ]);
					fclose($yourfile);
					$http_return_code = 200;
			}
			catch(Exception $e)
		...
		{
			$vegan_html = $_POST[ "html" ];
			$filename = "saved-templates/" . $_POST[ "filename" ] . ".backup";
			try{
					$myfile = fopen($filename, "w");
					fwrite($myfile, $vegan_html);
					fclose($myfile);
...
function ResizeImage( $file_name, $method, $width, $height )
{
	...
	if ( $method == "resize" )
	{
		...
		// Pad the side articles but not the banners
		if($width < 210)
		{
			$image->spliceImage(20, 0, $width, 0);
		}
