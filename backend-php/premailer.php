...
class Premailer {
	...
	public static function html($html, $fetchresult = true, $adaptor = 'hpricot', $base_url = '', $line_length = 65, $link_query_string = '', $preserve_styles = true, $remove_ids = false, $remove_classes = false, $remove_comments = false) {
		$clean = self::convert($html, '', $fetchresult, $adaptor, $base_url, $line_length, $link_query_string, $preserve_styles, $remove_ids, $remove_classes, $remove_comments);
    $clean = str_replace("<!--DontPreMailThis", "<!--DontPreMailThis-->", $clean);
    $clean = str_replace("/DontPreMailThis-->", "<!--/DontPreMailThis-->", $clean);
		$clean = str_replace("http://*|UNSUB|*", "*|UNSUB|*", $clean);
		$clean = str_replace("http://*|ARCHIVE|*", "*|ARCHIVE|*", $clean);
